...
import { useGetRecentTradesQuery, useGetMarketPricesQuery } from "../features/pacificaSlice";
...
// Per-symbol feed — polls every 5s
...
const WhaleFeed = () => {
  ...
  const allRaw = useMemo(() => [
    ...
    ...doge.trades, ...wif.trades, ...arb.trades,
  ...
  const { buyPct, sellPct } = useMemo(() => {
    const buyV = filtered.filter((t) => t.side === "BUY").reduce((a, t) => a + t.sizeUsd, 0);
    const sellV = filtered.filter((t) => t.side === "SELL").reduce((a, t) => a + t.sizeUsd, 0);
    const tot = buyV + sellV || 1;
    return { buyPct: ((buyV / tot) * 100).toFixed(1), sellPct: ((sellV / tot) * 100).toFixed(1) };
  }, [filtered]);
  ...
  useEffect(() => {
    ...
    const totalOI = priceData.reduce((a, d) => a + parseFloat(d.open_interest ?? 0), 0);
    const now = new Date();
  ...
  const topMarkets = useMemo(() =>
    [...priceData].sort((a, b) => parseFloat(b.volume_24h ?? 0) - parseFloat(a.volume_24h ?? 0)).slice(0, 5),
    [priceData]);
    ...
    <div className="w-full">
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="flex items-center gap-2 text-xs font-body text-positive">
            <span className="w-2 h-2 rounded-full bg-positive animate-pulse" /> Live
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3 mb-5 items-center">
        ...
        <div className="flex items-center gap-2 ml-auto">
          <span className="text-[10px] text-muted-foreground font-mono shrink-0">Min size:</span>
          <input type="range" min={500} max={200000} step={500} value={minSize}
            onChange={(e) => setMinSize(Number(e.target.value))}
            className="w-24 accent-primary" />
          <span className="text-[10px] font-mono text-primary w-16 shrink-0">{fmt(minSize)}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        ...
        <div className="col-span-3 card-gradient rounded-lg border border-border p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[10px] font-mono text-muted-foreground">{filtered.length} trades</span>
          </div>
          ...
          <div className="space-y-0.5 max-h-[500px] overflow-y-auto pr-1">
            {totalTradesLoaded === 0 ? (
              <div className="flex flex-col items-center justify-center h-40 gap-3">
                ...
                <span className="text-xs font-mono text-muted-foreground">Loading trade feed…</span>
              </div>
            ) : filtered.length === 0 ? (
                  ...
                  <motion.div key={t.id}
                    ...
                    className={`flex items-center gap-3 px-3 py-2 rounded text-xs font-mono
                      ${t.cause === "LIQUIDATION"
                        ...
                        : "hover:bg-muted/30 border-l-2 border-transparent"} transition-colors`}>
                    <span className="text-muted-foreground w-14 shrink-0">{timeAgo(t.timestamp)}</span>
                    ...
                    <span className={`flex items-center gap-0.5 w-16 shrink-0 ${
                    ...
                    <span className={`font-bold ${t.sizeUsd >= 100000 ? "text-foreground" : "text-muted-foreground"}`}>
                      {fmt(t.sizeUsd)}
                    </span>
                    {t.cause && (
                      <span className={`ml-auto text-[10px] px-1.5 py-0.5 rounded font-bold shrink-0 ${
                        t.cause === "LIQUIDATION"
        ...
        <div className="col-span-2 space-y-4">
          ...
          <div className="card-gradient rounded-lg border border-border p-4">
            ...
            <div className="flex h-5 rounded-full overflow-hidden mb-2">
              <div className="bg-positive transition-all duration-500" style={{ width: `${buyPct}%` }} />
              ...
              <div className="h-40">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={oiBuffer}>
                    <XAxis dataKey="time" tick={{ fill: "#5A6478", fontSize: 9 }}
                      axisLine={false} tickLine={false} interval={Math.floor(oiBuffer.length / 4)} />
                    ...
                    <Line type="monotone" dataKey="oi" stroke="#00D4FF" strokeWidth={1.5}
                  ...
                  <div key={m.symbol} className="flex items-center gap-2 text-xs">
                    ...
                    <div className="flex-1 h-1.5 bg-muted rounded-full overflow-hidden">
                      <div className="h-full bg-primary/50 rounded-full transition-all duration-700"
                        style={{ width: `${(parseFloat(m.volume_24h ?? 0) / maxVol) * 100}%` }} />
                    </div>
