  ...
  const handleRefresh = useCallback(() => {
    ...
    dispatch(
      pacificaApi.util.invalidateTags([
        "MarketInfo", "MarketPrices", "CandleData",
        "FundingRateHistory", "RecentTrades",
        "AccountInfo", "Positions", "TradeHistory",
      ])
    );
    dispatch(
      elfaApi.util.invalidateTags([
        "TrendingTokens", "TokenNews", "TopMentions", "TrendingCAs", "EventSummary",
      ])
    );
    ...
    <header className="flex items-center justify-between px-3 md:px-6 border-b h-14 border-border bg-card shrink-0">
      ...
      <div className="hidden md:flex items-center gap-3">
        ...
        <button
          ...
          title={sinceRefresh ? `Last refreshed ${sinceRefresh}` : "Refresh all data"}
      ...
      >
        {menuOpen ? <IoMdClose className="w-6 h-6" /> : <IoMenu className="w-6 h-6" />}
      </div>
    </header>
  );
};
